...
| Glial client runtime (persistence-first, assembly, rich events) | **v0 Built** (2026-07-10: glial 4270235+d94c49c — mount/unmount instances (idiom-agnostic), store-only path, value+log folds ORACLE-GATED (11/11 + 6 assembly vectors), injectable session seam; s-stack-multi trace 0938190) | `glial/GlialClientRuntime.md` + s-stack-* + `glial/dev-docs/DecisionLog.md` (GAP-1..5) | **T3 BUILT 2026-07-10**: grip adapter (glial a31587f, kernel stays grip-free) + per-binding cutover DONE (glade e2679d6..3dfc2e6 — GripShareBinder DELETED, decl/manifest plumbing remains, coverage 12→17 with per-binding wire-byte interop proofs). OPEN: GAP-9 reload-resume (glial-native session hydration + real GC-4 store engine — demo-grade workaround in place, silent own-chain fork was observed live) |
