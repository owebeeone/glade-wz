...
| `grazel1/p3-heavy-shapes` | P3 | files (SWMR-backed window view + blobs) + live terminal (WINCH) in tabs | released SWMR corpus remains green; exact Glade node/client/Glial adapter gates pass; file view never mixes generations; terminal live session survives resize; large-binary path never chains ops | **in progress** — P3.S1 transport/assembly vertical slice built 2026-08-29; full path viewport/backfill, blobs, supplier, and terminal remain |
| `grazel1/p4-editing` | P4 | collaborative editing in gryth-ui | released CRDT + `text_crdt` corpora remain green; exact Glade/Glial integration gates pass; cursor-stable remote deltas and convergence live | pending |
