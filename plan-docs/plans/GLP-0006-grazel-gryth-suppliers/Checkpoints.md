...
| `grazel1/p3-heavy-shapes` | P3 | files (SWMR-backed window view + blobs) + live terminal (WINCH) in tabs | released SWMR corpus remains green; exact Glade node/client/Glial adapter gates pass; file view never mixes generations; terminal live session survives resize; large-binary path never chains ops | pending |
