...
| `GLP-0006` | grazel + gryth — the glade supplier program | active (P1 met; P3.S1 SWMR vertical slice built) | maintainer (coordinating) | glade-wz root on `main`; future tags `grazel1/*` | `grazel`, `glade-chat`/`glade-gwz`/`glade-files`/`glade-terminal`/`glade-editing`, `glade` (F1, channels, exact SWMR/CRDT adapters, stage-2), `glial` (typed manifest, view/delta integration), released `taut-shape 0.9.*` contracts/engines, `ggg-viz`, gryth-wz/`gryth-ui` |
